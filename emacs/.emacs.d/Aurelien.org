...
* Setup
  ...
  Emacs modes have the tendency to be pretty aggressive when it comes
  to binding keys. Instead of relying on Emacs built-in
  ~global-set-key~ and ~local-set-key~ I prefer to have my own key-map
  with a ~defadvice~ to ensure my keys are not overridden.
  #+begin_src emacs-lisp
