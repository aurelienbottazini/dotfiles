...
* NerdTree || File & Folders

  For icons you need to install fonts available here:
  https://github.com/domtronn/all-the-icons.el/tree/master/fonts
#+begin_src emacs-lisp
(use-package neotree
:config
(use-package all-the-icons)
(define-key my-keys-minor-mode-map (kbd "M-\\") 'neotree-toggle)
(define-key my-keys-minor-mode-map (kbd "M-|") 'neotree-find)
...
** Tmux
   #+BEGIN_SRC emacs-lisp
     ...
     (defun tmux-move-left ()
       ...
       (if window-system
           (evil-window-left 1)
         (shell-command (concat (tmux-socket-command-string) " select-pane -L"))))
     (defun tmux-move-down ()
       (interactive)
       (if window-system
           (evil-window-down 1)
         (shell-command (concat (tmux-socket-command-string) " select-pane -D"))))
     (defun tmux-move-up ()
       (interactive)
       (if window-system
           (evil-window-up 1)
           (shell-command (concat (tmux-socket-command-string) " select-pane -U"))))
     (defun tmux-move-right ()
       (interactive)
       (if window-system
           (evil-window-right 1)
           (shell-command (concat (tmux-socket-command-string) " select-pane -R"))))
...
* Colors
  #+begin_src emacs-lisp
    ...
    ;;   :config
    ;;   (load-theme 'leuven))
    ;; (set-face-attribute 'fringe nil :background "white" :foreground "#6e93d3")
    ;; (if window-system
    ;;     (progn
    ;;       (use-package leuven-theme
    ;;         :config
